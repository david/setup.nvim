...
local traits = {}
local function define_trait_component(name)
  _G.assert((nil ~= name), "Missing argument name on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:21")
  traits[name] = {}
  return nil
end
local function ensure_trait_component(name)
  _G.assert((nil ~= name), "Missing argument name on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:24")
  return assert(traits[name], ("no trait component: " .. name))
end
local function define_trait(component, trait_name, config)
  _G.assert((nil ~= config), "Missing argument config on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:27")
  _G.assert((nil ~= trait_name), "Missing argument trait-name on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:27")
  _G.assert((nil ~= component), "Missing argument component on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:27")
  ensure_trait_component(component)
  assert(config.fn, ("no trait function: " .. trait_name))
  return table.insert(traits[component], {trait_name, config})
end
local function call_traits(component_name, ...)
  local rest = {...}
  _G.assert((nil ~= rest), "Missing argument rest on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:32")
  _G.assert((nil ~= component_name), "Missing argument component-name on /var/home/david/Worktrees/setup.nvim/fnl/setup/init.fnl:32")
  ensure_trait_component(component_name)
  for _, _6_ in ipairs(traits[component_name]) do
    local _0 = _6_[1]
    local trait = _6_[2]
...
local function keymap_trait(_7_)
...
define_trait("nvim", "colorscheme", {fn = _13_})
...
define_trait("nvim", "opt", {fn = _20_})
define_trait("nvim", "keymap", {fn = keymap_trait})
define_trait_component("plugin")
...
define_trait("plugin", "ensure", {fn = _21_})
...
define_trait("plugin", "opt", {fn = _23_})
define_trait("plugin", "keymap", {fn = keymap_trait})
define_trait_component("filetype")
local function _28_(_27_, ft)
  ...
  if _3fconfig then
    ...
    for key, val in pairs((_3fconfig or {})) do
      local function _29_()
        return call_traits("plugin", val, key)
      end
...
define_trait("filetype", "plugin", {fn = _28_})
...
define_trait("filetype", "lang", {fn = _31_})
...
define_trait("filetype", "conform", {fn = _35_})
local function setup(name, _3fconfig)
  ...
  local config = (_3fconfig or {})
  if (name == "nvim") then
    return call_traits("nvim", config)
  elseif ((_G.type(name) == "table") and (name[1] == "filetype") and (nil ~= name[2])) then
    local ft = name[2]
    return call_traits("filetype", config, ft)
  else
    local _ = name
    return call_traits("plugin", config, name)
  end
end
return {setup = setup}
